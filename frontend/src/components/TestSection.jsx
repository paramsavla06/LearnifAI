...
import {
    ...
    CheckCircle, XCircle, BarChart2, Brain, AlertTriangle, Star, Loader2, Hash, Target
} from 'lucide-react'
...
// Count available questions for a subject + semester
function countAvailableQuestions(subjectName, sem) {
    const subj = conceptsData.subjects.find(s => s.name === subjectName)
    if (!subj) return 0
    let count = 0
    for (const concept of subj.concepts) {
        if (concept.semester !== sem) continue
        count += (questionsBySlug[concept.slug] || []).length
    }
    return count
}
...
const STEPS = [
    ...
    { id: 2, label: 'Questions', icon: Target },
    { id: 3, label: 'Diagnostic', icon: Brain },
...
function SetupStep({ profile, setProfile, onNext }) {
    ...
    const handleYearChange = (y) => {
        ...
        setProfile(p => ({ ...p, year: y, semester: newSems[0] || '', selectedSubject: '' }))
    }
    ...
    return (
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={e => setProfile(p => ({ ...p, semester: e.target.value, selectedSubject: '' }))}
            ...
            >
                Choose Subject & Questions <ChevronRight className="w-5 h-5" />
            </button>
        </motion.div>
    )
}

// ─── Step 2 — Subject + Question Count Selection ──────────────────────────────
function QuestionSetupStep({ profile, setProfile, onNext, onBack }) {
    const [err, setErr] = useState('')
    const sem = profile.semester
    const availableSubjects = getSubjectsForSem(sem)
    const QUESTION_OPTIONS = [3, 5, 10]

    const selectedSubject = profile.selectedSubject || ''
    const questionCount = profile.questionCount || 5

    const maxQuestions = useMemo(() => {
        if (!selectedSubject) return 0
        return countAvailableQuestions(selectedSubject, sem)
    }, [selectedSubject, sem])

    // Filter question options to those that are actually possible
    const validOptions = QUESTION_OPTIONS.filter(n => n <= maxQuestions)

    function handleSelectSubject(name) {
        const max = countAvailableQuestions(name, sem)
        // Auto-select a valid question count
        let qc = questionCount
        if (qc > max) {
            qc = QUESTION_OPTIONS.filter(n => n <= max).pop() || max
        }
        setProfile(p => ({ ...p, selectedSubject: name, questionCount: qc }))
    }

    function handleNext() {
        if (!selectedSubject) {
            setErr('Please select a subject.')
            return
        }
        if (!questionCount || questionCount > maxQuestions) {
            setErr(`Please select a valid number of questions (max ${maxQuestions} available).`)
            return
        }
        setErr('')
        onNext()
    ...
    return (
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            <h3 className="text-2xl font-bold text-white mb-2">Choose Your Quiz</h3>
            <p className="text-text-secondary font-medium mb-6">
                Pick a subject and how many questions you want to attempt.
            </p>
            ...
            <div className="mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {availableSubjects.map(s => {
                        const qCount = countAvailableQuestions(s.name, sem)
                        ...
                        return (
                            <button
                                ...
                                className={`text-left p-4 rounded-xl border transition-all duration-200 ${
                                    isSelected
                                        ? 'bg-primary-accent/10 border-primary-accent shadow-[0_0_24px_rgba(255,216,95,0.12)]'
                                        : 'border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/8'
                                }`}
                            >
                                <div className="flex items-center justify-between mb-1">
                                    <span className={`text-sm font-bold ${isSelected ? 'text-primary-accent' : 'text-white'}`}>
                                        {s.name}
                                    </span>
                                ...
                                <p className="text-[11px] text-text-secondary leading-relaxed">{s.description}</p>
                            </button>
                        )
                    })}
                </div>
                ...
                >
                    ...
                    <div className="flex gap-3">
                        {QUESTION_OPTIONS.map(n => {
                            const isAvailable = n <= maxQuestions
                            const isActive = questionCount === n && isAvailable
                            return (
                                <button
                                    key={n}
                                    onClick={() => isAvailable && setProfile(p => ({ ...p, questionCount: n }))}
                                    disabled={!isAvailable}
                                    className={`flex-1 flex flex-col items-center gap-2 py-4 rounded-xl border font-bold text-sm transition-all duration-200 ${
                                        isActive
                                            ? 'bg-primary-accent/10 border-primary-accent text-primary-accent shadow-[0_0_20px_rgba(255,216,95,0.15)]'
                                            : isAvailable
                                                ? 'border-white/10 text-text-secondary bg-white/5 hover:border-white/20 hover:text-white cursor-pointer'
                                                : 'border-white/5 text-text-secondary/30 bg-white/[0.02] cursor-not-allowed'
                                    }`}
                                >
                                    <Hash className="w-5 h-5" />
                                    <span className="text-lg">{n}</span>
                                    <span className="text-[10px] uppercase tracking-widest">
                                        {n === 3 ? 'Quick' : n === 5 ? 'Standard' : 'Deep'}
                                    </span>
                                </button>
                            )
                        })}
                    ...
                    {maxQuestions > 0 && (
                        <p className="text-[11px] text-text-secondary mt-2">
                            {maxQuestions} questions available in {selectedSubject} for {sem}
                        </p>
                    )}
                </motion.div>
            )}
            ...
            {selectedSubject && questionCount && (
                <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="mb-6 p-4 rounded-xl bg-black/30 border border-primary-accent/20"
                >
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-primary-accent/10 border border-primary-accent/20 flex items-center justify-center">
                            <Zap className="w-5 h-5 text-primary-accent" />
                        </div>
                        <div>
                            <p className="text-sm font-bold text-white">
                                {questionCount} questions from {selectedSubject}
                            </p>
                            <p className="text-[11px] text-text-secondary">
                                {questionCount <= 3 ? 'Quick check — great for revision' : questionCount <= 5 ? 'Standard diagnostic — balanced coverage' : 'Deep diagnostic — thorough concept analysis'}
                            </p>
                        </div>
                    </div>
                </motion.div>
            ...
            <div className="flex gap-3">
                <button
                    onClick={onBack}
                    className="flex items-center justify-center gap-2 px-6 py-4 rounded-full border border-white/20 text-text-secondary font-bold text-sm hover:bg-white/5 hover:text-white transition-all"
                >
                ...
                <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 px-6 py-4 rounded-full bg-primary-accent text-black font-bold text-sm hover:opacity-90 transition-opacity shadow-[0_0_30px_rgba(255,216,95,0.3)] disabled:opacity-40 disabled:cursor-not-allowed"
                >
                    Start Quiz ({questionCount} Qs) <ChevronRight className="w-5 h-5" />
                </button>
            </div>
        </motion.div>
    )
...
function DiagnosticStep({ profile, onFinish }) {
    ...
    const sem = profile.semester
    const subjectName = profile.selectedSubject

    const [questions, setQuestions] = useState(() => pickQuestions(subjectName, sem, qCount))
    ...
    if (!q) return (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
            ...
            <p className="text-text-secondary font-medium">No questions available for this subject and semester.</p>
        </div>
    )
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
            <div className="mb-6">
                <div className="flex justify-between text-xs font-bold text-text-secondary mb-2">
                    <span>Q{qIdx + 1} of {totalQs} — <span className="text-primary-accent">{subjectName}</span></span>
                    <span>{Math.round((qIdx + 1) / totalQs * 100)}%</span>
                </div>
...
export default function TestSection() {
    ...
    // If logged in, skip Step 1 but still show Step 2 for subject/question selection
    useEffect(() => {
        if (profile._fromSession) setStep(2)
    }, [])

    function handleDiagnosticFinish(data) { setResult(data); setStep(4) }
    function handleRetake() {
        setStep(2)
        setResult(null)
        setProfile(p => ({ ...p, selectedSubject: '' }))
    }
            ...
            <div className="relative max-w-7xl mx-auto px-6 md:px-8">
                ...
                <ScrollReveal className="mb-12 origin-left">
                    ...
                    <p className="text-text-secondary mt-4 max-w-xl text-lg font-medium">
                        Choose your subject, pick 3, 5, or 10 questions, and get an instant diagnostic.
                        Our BKT engine scores each concept and maps weak areas to your library.
                    </p>
                </ScrollReveal>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    <div className="lg:col-span-8">
                        <GlassCard className="!p-8">
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {step === 2 && (
                                    <QuestionSetupStep key="question-setup" profile={profile} setProfile={setProfile} onNext={() => setStep(3)} onBack={() => setStep(1)} />
                                )}
                                {step === 3 && (
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <GlassCard className="!p-6">
                            ...
                            {[
                                { icon: User, label: 'Setup', desc: 'Enter your profile and semester' },
                                { icon: Target, label: 'Choose', desc: 'Pick a subject and number of questions (3, 5, or 10)' },
                                { icon: Brain, label: 'Diagnostic', desc: 'Answer adaptive MCQs from your chosen subject' },
